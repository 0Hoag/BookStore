...
public interface IdentityClient {
        ...
        ApiResponse<SelectedProductResponse> getSelectedProduct(@PathVariable("SelectedId") String SelectedId, @RequestHeader("Authorization") String token);

        @GetMapping(value = "/order/{orderId}", produces = MediaType.APPLICATION_JSON_VALUE)
        ApiResponse<OrdersResponse> getOrder(@PathVariable("orderId") String orderId, @RequestHeader("Authorization") String token);
}
